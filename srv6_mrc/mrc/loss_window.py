"""Per-flow per-EV data-loss accounting (receiver side).

Tracks counts of received data packets per (FlowKey, plane, path) over a
rolling time window — one (plane, path) cell is an "EV" in MRC parlance.
On window close, the receiver-side I/O layer calls `snapshot_and_reset()`
to get a `LossReport` ready to wire-encode and unicast back to the sender.

Sender-driven attribution
-------------------------
Per-EV loss can't be cleanly computed receiver-side without knowing how
many packets the sender intended for each EV in this window. Instead the
wire model is:

    receiver -> sender:  per-EV (seen, expected_local, max_gap)
    sender computes:     loss_ratio[ev] = 1 - seen[ev] / sent[ev]

where `sent[ev]` comes from the sender's own per-window emit counters.
The receiver still fills in `expected_local = max_seq - min_seq + 1`
(the flow-seq span observed on that EV) so the sender has a usable
...
sprayed across EVs, max_seq - min_seq + 1 is a STRICT UPPER BOUND on
how many packets sender sent on this EV in the time window. The actual
number is between `seen[ev]` and that upper bound. This is not great
in low-loss conditions (we'd never demote) but the sender prefers its
own `sent[ev]` counter when present.

`max_gap` is the largest jump between consecutive seqs received on an
EV, useful as an out-of-order / burst-loss signal that's independent
...
O(planes * paths) per flow; even with hundreds of flows the wall-clock
cost stays in the microseconds range.
"""
...
@dataclass
class _EvCounters:
    """Per-EV counters for one in-flight window of one flow."""
    seen: int = 0
...
class _FlowWindow:
    ...
    `evs` is a 2-D grid indexed [plane][path]; built lazily on first
    record() since most flows will only touch a few EVs.
    """
    next_window_id: int = 0
    evs: List[List[_EvCounters]] = field(default_factory=list)

    def reset(self, num_planes: int, num_paths: int) -> None:
        # Keep next_window_id; reset everything else.
        self.evs = [
            [_EvCounters() for _ in range(num_paths)]
            for _ in range(num_planes)
        ]


class LossWindowTable:
    """Per-(FlowKey, plane, path) data-packet counters, sliced into windows.

    The receiver hooks `record(flow_key, plane, path, seq)` into its
    data RX loop. A separate thread (or timer) calls
    `snapshot_and_reset(flow_key)` at the end of each window to emit a
    LossReport carrying one PlaneLossRecord per active EV.
    ...
    def __init__(self, *, num_planes: int, num_paths: int) -> None:
        if num_planes <= 0:
    ...
    def record(self, flow_key, plane: int, path: int, seq: int) -> None:
        """Account for one received data packet on EV (plane, path).
        ...
        `path` is the spine_id the sender chose for this packet's
        plane (==MRC path_id; from the data payload's path field).

        `seq` is the flow-global monotonic sequence number from the
        data payload. We assume sender-side seq is non-decreasing per
        flow; out-of-order RX (which happens with multi-EV spray) is
    ...
    def snapshot_and_reset(self, flow_key) -> LossReport:
        ...
        Returns a LossReport with one PlaneLossRecord per active EV in
        (plane, path) order. If the flow has never been seen, returns
        an empty report with window_id=0 (caller may choose to skip
        emission).
